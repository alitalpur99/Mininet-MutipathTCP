...
with open('h1dump.txt','r') as f1:
	for line in f1:
		...
		# 	# line=line[4:]
			if line[11]=='38324' or line[10]=='38324'  or line[8]=='38324' or line[9]=='38324':
				# print line[1]
		 		time1.append(line[2])
...
with open('h4dump.txt','r') as f2:
	for line in f2:
		...
		if line[3]=='10.0.0.1':# and :
		# 	# line=line[4:]
			if line[10]=='38324' or line[11]=='38324'  or line[8]=='38324' or line[10]=='38324'or line[13]=='38324':
				# print line[4]
		 		time2.append(line[2])
...
with open("h4-timeseq") as f4:
	for l in f4:
		with open("h1-timeseq") as f1:
			l=l.strip()
			t2=l.split(" ")[0] # time field from h4dump.txt
			s2=l.split(" ")[1]	# seq field from h4dump.txt
			# print l
# 			for i in range(len(time2)):
			for line in f1:
				line =line.strip()
				t1=line.split(" ")[0]	# time field from h1dump.txt
				s1=line.split(" ")[1]	#  seq number field from h1dump.txt


				## when sequence number of both hosts are equal append these time-seq values
...
print "timeh1 length",len(time_h1)
print "timeseqlen", len(timeseq)
...
with open("h1h4timeseqok") as f:
	...
	with open('tcpdump-timedelay','w+') as file:
		for n in range(len(one_way_delay)):
			...
			delay=d.split(":")[2]
			if n>0:
				file.write(str(time2[n])+" "+str(delay)+"\n")
